...
namespace kyr
{
	HRESULT CSound::Load(const std::wstring& path)
	{
		if (nullptr == CSoundManager::GetDevice())
	...
	bool CSound::LoadWavFile(const std::wstring& path)
	{
		...
		if (FAILED(CSoundManager::GetDevice()->CreateSoundBuffer(&mBufferDesc, &mSoundBuffer, NULL)))
		{
	...
	void CSound::SetPosition(float position, bool loop)
	{
		Stop(true);
